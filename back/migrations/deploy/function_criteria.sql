	...
	AND (CASE
        -- Test for the presence of the type_ecole key and array in the json
	  	WHEN (($1->'type_ecole')::text IS NOT NULL) = true AND (json_typeof($1->'type_ecole')::text) = 'array'
		  	...
		  	THEN private.commune.code_insee IN (SELECT private.school.commune_code FROM private.school WHERE private.school.type IN (SELECT json_array_elements_text($1->'type_ecole')))
	  	ELSE true 
	  	END)
		AND (CASE
		...
		WHEN (($1->'type_personal_health')::text IS NOT NULL) = true AND (json_typeof($1->'type_personal_health')::text) = 'array'
			THEN private.commune.code_insee IN (SELECT private.personal_health.commune_code FROM private.personal_health WHERE private.personal_health.profession IN (SELECT json_array_elements_text($1->'type_personal_health')))
		ELSE true 
		END)
		AND (CASE
		-- Test for the presence of the type_commerce key and array in the json
		WHEN (($1->'type_health_institution')::text IS NOT NULL) = true AND (json_typeof($1->'type_health_institution')::text) = 'array'
			THEN private.commune.code_insee IN (SELECT private.health_institution.commune_code FROM private.health_institution WHERE private.health_institution.categorie IN (SELECT json_array_elements_text($1->'type_health_institution')))
		ELSE true 
		END)
$$ LANGUAGE SQL STRICT;
