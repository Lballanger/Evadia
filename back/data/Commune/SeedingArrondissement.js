...
const getCommune = async () => {
    const result = await fetch(`https://opendata.paris.fr/api/records/1.0/search/?dataset=arrondissements&q=&rows=66&facet=c_ar&facet=c_arinsee&facet=l_ar`);
    return result.json();
}
...
const insertBDD = async () => {
    ...
    try {
        ...
        for (const value of data.records) {
            ...
            const coordinates = value.fields.geom_x_y.map((coord) => parseFloat(coord));
            //console.log(coordinates);
            //fields.l_ar -> nom
            //fields.c_arinsee -> code insee
            //coordinates
                    const postal = value.fields.c_arinsee-100;
                    await db.query(`INSERT INTO private.commune (
                        code_insee,
                        city_name,
                        coordinates,
                        code_postal
